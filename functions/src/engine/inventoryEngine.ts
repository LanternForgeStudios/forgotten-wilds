 ...
 * one) - the "grant an item" mutation shared by shop purchases, chests, combat loot, and quest
 * rewards, so the unique-item cap is enforced identically everywhere an item can be granted.
 * Returns `false` (and leaves `inventory` untouched) if this would duplicate a unique item; the
 * caller decides how to react - throw (a deliberate purchase), or silently skip (loot/rewards,
 * where duplicating isn't the player's fault and shouldn't surface as an error).
 */
export function grantItem(inventory: InventoryItem[], itemId: string, quantity = 1): boolean {
